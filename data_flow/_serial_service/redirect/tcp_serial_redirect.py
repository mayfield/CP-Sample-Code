...
class Redirector:
    ...
    def reader(self):
        ...
        while self.alive:
            try:
                # TODO - due to the 1100's FIFO, this causes 8 bytes per network packet
                ...
                if data:
                    ...
                    if self.spy:
                        data = codecs.escape_encode(data)[0]
                        str_data = data.decode("utf-8")
                        sys.stdout.write(str_data)
                        sys.stdout.flush()

                    # then do the NL changes
        ...
        while self.alive:
            try:
                ...
                if self.ser_newline and self.net_newline:
                    ...
                    # XXX fails for CR+LF in input when it is cut in half at the begin or end of the string
                    str_data = data.decode("utf-8")
                    data = ser_newline.join(str_data.split(net_newline))
...
""")
    ...
    group = optparse.OptionGroup(parser,
                                 "Newline Settings",
                                 "Convert newlines between network and serial port. Conversion is normally" +
                                 "disabled and can be enabled by --convert."
                                 )
    parser.add_option_group(group)
    ...
    while True:
        try:
            ...
            # this spawns a sub-process in class Redirector() 
            r.shortcut()
            if options.spy:
                sys.stdout.write('\n')
