...
['user_tag'] The user asset tag, which is unicode. The user asset tag is a user-assigned tag, such
        as "NE16273", which is used to identify something in other systems. It likely must be
...
class DataBase(DataCore):
    ...
    # the pointer to the ROOT object, whose child list ultimately holds every other object
    _DATA_ROOT = None

    # _DATA_SETTINGS is a special case of object, as the values are assumed not to change often,
    # plus MUST be held locally in non-volatile storage
    _DATA_SETTING = None

    # root template, acts as parent for all 'templates'
    _DATA_TEMPLATE = None

    # a simple list of objects, which can be processed in a loop; order defined by creation time
    _DATA_ARRAY = []

    # These are what API element paths begin with, so like "data/tank/0/alarm"
    API_BASE_NAME_LIST = ('data', 'templates', 'settings', 'status', 'config', 'state', 'control')
    ...
    def find_template_object(self, name: str):
        """
        Search for an object based on name, limiting search to the db_object._DATA_TEMPLATES list
    ...
    def find_setting_object(self, name: str):
        """
        Search for an object based on name, limiting search to the db_object._DATA_SETTINGS list
    ...
    def find_object(self, name: str):
        ...
        :return:
        """
        # this throws error is anything is wrong with the name, plus forces case, etc
        if self._DATA_ROOT is not None:
    ...
    def add_to_data_base(self, node, overwrite=False):
        ...
        :param node: the node to add, must be derived from DataCore (won't be None - just include
        ...
        if role == "setting":
            ...
            DataBase.logger.debug("Add Template({0})".format(node.get_name()))
            ...
            if node.get_parent() is None:
                ...
                DataBase.logger.debug("Add Data({0})".format(node.get_name()))
    ...
    def split_object_path(self, path: str):
        """
        Given a path such as "/data/tank/0/alarm/hialm", return a list of the base names,
        such as ["data", "tank", "0", "alarm", "hialm"] Any leading or trailing empty names
        are discarded, plus filesystem names such as "C:", "users", or "api" are also deleted.

        Note that this routine does NOT validate any of the names! This is merely the splitters.
        """
    ...
    def fetch_object_path_list(self, path: str):
        """
        Given a path such as "/data/tank/0/alarm/hialm", return a list of the DATA object
        references. See split_object_path() to understand how split.
        """
