...
def name_to_hex(name, spec='css3'):
    ...
    Traceback (most recent call last):
        ...
    TypeError: 'css5' is not a supported specification for color name lookups;
    supported specifications are: html4, css2, css21, css3.

    """
    if spec not in SUPPORTED_SPECIFICATIONS:
        raise TypeError(
            "'%s' is not a supported specification for color name " % spec +
            "lookups; supported specifications are: %s." %
...
def hex_to_name(hex_value, spec='css3'):
    ...
    """
    if spec not in SUPPORTED_SPECIFICATIONS:
        raise TypeError("'%s' is not a supported specification for" % spec +
                        "color name lookups; supported specifications" +
                        " are: %s." % ', '.join(SUPPORTED_SPECIFICATIONS))
    normalized = normalize_hex(hex_value)
    try:
        name = globals()['%s_hex_to_names' % spec][normalized]
    except KeyError:
        raise ValueError("'%s' has no defined color name in %s." % (hex_value,
                                                                    spec))
    return name
...
def hex_to_rgb(hex_value):
    ...
    return tuple([int(s, 16) for s in (hex_digits[1:3], hex_digits[3:5],
                                       hex_digits[5:7])])
...
def rgb_percent_to_name(rgb_percent_triplet, spec='css3'):
    ...
    """
    return rgb_to_name(rgb_percent_to_rgb(
        normalize_percent_triplet(rgb_percent_triplet)), spec=spec)
...
def rgb_percent_to_hex(rgb_percent_triplet):
    ...
    """
    return rgb_to_hex(rgb_percent_to_rgb(
